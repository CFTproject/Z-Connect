...
response = WS.sendRequest(findTestObject('Convert Currency/Rate type Tran type Conversion table ID'))
...
for(int i=3; i<4; i++)
{
	...
	WS.verifyElementPropertyValue(response, str+'conversionTableId', testdatapath.getValue(8, i))
	...
	WS.verifyElementPropertyValue(response, str+'rateType', testdatapath.getValue(11, i))
	
	WS.verifyElementPropertyValue(response, str+'tranType', testdatapath.getValue(12, i))
